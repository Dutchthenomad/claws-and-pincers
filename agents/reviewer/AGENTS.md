...
- **#task-dispatch** — Awareness of what tasks are in flight (you don't pick up tasks from here — you pick up from #review-queue).
- **#review-queue** — Your primary intake channel. Completed deliverables land here for your review.
...
1. Check #review-queue for deliverables awaiting review
...
3. **Before reviewing, gather context:**
   ...
   - Read `anti-patterns.md` for patterns to watch for
...
3. **Conflict detection completed?** — Verify that conflict detection was run before the task was dispatched. Check `conflict-registry.json` for the project.
4. **Anti-patterns consulted?** — The specialist should have consulted `anti-patterns.md` before starting. If the deliverable shows signs of known anti-patterns, flag it.
...
Post the verdict to #review-verdicts. Route to the appropriate severity channel per the notification matrix:
...
2. Write the entry to `anti-patterns.md` using the standard format:
```
...
```
3. Notify Orchestrator via `sessions_send` that a new anti-pattern has been logged
...
2. Check #review-queue for any pending reviews
3. Read `anti-patterns.md` for current institutional knowledge
4. Resume any in-progress reviews or begin the highest-priority pending review
